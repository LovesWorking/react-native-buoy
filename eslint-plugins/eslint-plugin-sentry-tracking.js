  ...
  rules: {
    'require-ignore-label': {
      meta: {
        type: 'problem',
        docs: {
          description: 'Enforce sentry-label prop with "ignore" prefix on interactive elements',
          category: 'Best Practices',
          recommended: true,
        },
        messages: {
          missingSentryLabel: '{{ component }} must have a sentry-label prop starting with "ignore" for tracking user interactions in dev tools',
          invalidSentryLabel: '{{ component }} sentry-label must start with "ignore" to exclude dev tools interactions from analytics',
        },
        schema: [], // no options
      },
      create(context) {
        ...
        function checkSentryLabel(node, componentName) {
          ...
          const sentryLabelAttr = node.attributes.find(
            (attr) => attr.type === 'JSXAttribute' && attr.name.name === 'sentry-label',
          );
          ...
          if (sentryLabelAttr.value) {
            ...
            } else if (sentryLabelAttr.value.type === 'JSXExpressionContainer' && 
            ...
            if (typeof labelValue === 'string' && !labelValue.toLowerCase().startsWith('ignore')) {
              context.report({
                node,
          ...
          JSXOpeningElement(node) {
            ...
            if (componentName === 'TextInput') {
              const hasAccessibilityLabel = node.attributes.some(
                (attr) => attr.type === 'JSXAttribute' && attr.name.name === 'accessibilityLabel',
              );
