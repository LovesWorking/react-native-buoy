...
```
I’m working in the rn-monorepo-clean project. I just created a new package called **@react-buoy/<your-package>** inside `packages/<your-package>`.
...
3. If this package should re-export shared UI or utils:
   ...
   - Replace any deep relative imports in other packages with the new alias (e.g. `@react-buoy/shared-ui/<feature>`).
...
6. Run `pnpm --filter @react-buoy/<your-package> typecheck` (and any other relevant filters) to confirm the new import paths resolve.
...
8. Summarise changes clearly and propose any follow-up builds (`pnpm --filter @react-buoy/shared-ui build`, etc.).
