          ...
          <View style={styles.headerLeft}>
            ...
            <View style={styles.typeIndicator}>
              {(() => {
                ...
                return <IconComponent size={12} color={getTypeColor(entry.type)} />;
              })()}
              <Text style={[styles.typeText, { color: getTypeColor(entry.type) }]}>{entry.type}</Text>
            </View>

            {/* Level indicator */}
            <View style={[styles.levelDot, getLevelDotStyle(entry.level)]} />
            <Text style={[styles.levelText, { color: getLevelTextColor(entry.level) }]}>
...
export const renderLogEntry: ListRenderItem<ConsoleTransportEntry> = ({ item: entry }) => {
