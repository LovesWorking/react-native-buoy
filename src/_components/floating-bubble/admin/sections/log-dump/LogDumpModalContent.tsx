...
import { FileText, FlaskConical, RefreshCw, Trash, X } from 'lucide-react-native';
...
export function LogDumpModalContent({ onClose }: LogDumpModalContentProps) {
  ...
  const calculateEntries = () => {
    ...
    const uniqueEntries = rawEntries.reduce((acc: ConsoleTransportEntry[], entry: ConsoleTransportEntry) => {
    ...
    return uniqueEntries.sort((a: ConsoleTransportEntry, b: ConsoleTransportEntry) => b.timestamp - a.timestamp);
  };
  ...
  const generateTestLogs = async () => {
    ...
    // Add test logs
    testLogger.debug('Testing debug functionality', { category: 'debug' });
      ...
      {selectedEntry ? (
        ...
        <>
          ...
          {getFilteredEntries().length === 0 ? (
            <View style={styles.emptyContainer}>{entries.length === 0 ? <EmptyState /> : <EmptyFilterState />}</View>
          ) : (
            <FlatList
              sentry-label="ignore log entries list"
              ref={flatListRef}
              data={getFilteredEntries()}
              renderItem={({ item }) => <LogEntryItem entry={item} onSelectEntry={selectEntry} />}
